    ...
    <DashboardLayout pageTitle="Dashboard">
      {!isLoading ? (
        <div className="w-full md:pl-[260px] pt-4 px-5 bg-neutral-100 min-h-screen overflow-x-scroll pr-5">
          ...
          <section className="flex md:flex-row flex-col mt-8 gap-4 items-start">
            {/* Students With Critical Issues */}
            <div className="w-full bg-white p-5 rounded md:w-1/2 h-fit table__container text-sm leading-4 pb-[4rem]">
              <h6 className="py-2 font-bold">Students With Critical Issues</h6>
              {performance?.studentsWithCriticalIssues?.length > 0 && (
                <Bar data={studentsCriticalData} options={chartOptions} />
              )}
              {performance?.studentsWithCriticalIssues?.length <= 0 && (
                <EmptyTable title="No students with low attendance records" />
              )}
              {isLoading && (
            ...
            <div className="w-full bg-white p-5 rounded md:w-1/2 h-fit table__container text-sm leading-4 pb-[4rem]">
              <h6 className="py-2 font-bold">Top Departments</h6>
              {performance?.topPerformingDepartments?.length > 0 && (
                <Bar data={topDepartmentsData} options={chartOptions} />
              )}
              {performance?.topPerformingDepartments?.length <= 0 && (
                <EmptyTable title="No departments with high attendance records" />
              )}
              {isLoading && (
                <div className="flex flex-col items-center justify-center w-full h-full">
                  <LoaderIcon />
